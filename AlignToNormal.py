...
for i in range(numberOfSelectedFacesFaces):
    ...
    #place a cone to the face's center
    ...
    '''Now the rotation-orientation bit..'''
    ...
    fnormalNormalized=[]
    #normalize vector from point on sphere to origin of sphere
    ...
    ''' make sure normal x,y,z values stay within the  acos domain boundaries -1..1'''
    ...
    #find the angle between the face normal and the upvector
    ...
    #calculcate euler angles based on axis-angle
    '''
    s=math.sin(angle)
    c=math.cos(angle)
    t=1-c
    ...
    #bank=x, heading=y, attitude=z
    bank,heading,attitude = toEuler(axisNormalized[0],axisNormalized[1],axisNormalized[2],angle)
        
    #convert to degrees (cause maya's rotate function expects degress instead of radians)
    heading*=(180/math.pi)
    ...
    #spherical coordinates calculation r,fi,theta
    r = math.sqrt(fnormal[0]*fnormal[0] + fnormal[1]*fnormal[1] + fnormal[2]*fnormal[2])
    fi = (math.acos(fnormal[1])/r) * (180.0/math.pi) # acos(y/r)
    theta = math.atan2(fnormal[0],fnormal[2]) * (180.0/math.pi) #atan2(x,z)) 
